#!/bin/bash
# Optimized version of build_android_library.sh for faster builds
...
build_android_native_library() {
  ...
  # Use all 16 cores for maximum parallelism
  CMAKE_JOBS="${CMAKE_JOBS:-16}"
...
build_aar() {
  ...
  ANDROID_HOME="${ANDROID_SDK:-/home/ubuntu/Android}" ./gradlew build \
    --daemon \
    --parallel \
    --max-workers="${GRADLE_MAX_WORKERS:-16}" \
    --configure-on-demand

  # Skip unit tests if SKIP_TESTS is set
  if [ "${SKIP_TESTS:-0}" != "1" ]; then
    ANDROID_HOME="${ANDROID_SDK:-/home/ubuntu/Android}" ./gradlew :executorch_android:testDebugUnitTest \
      --daemon \
      --parallel \
      --max-workers="${GRADLE_MAX_WORKERS:-16}"
  fi
...
# Build ABIs in parallel if requested
